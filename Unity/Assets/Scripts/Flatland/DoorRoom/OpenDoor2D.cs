...
public class OpenDoor2D : MonoBehaviour
{
    ...
    void Update()
    {
        isClosing = closingDoorScript.GetComponent<CloseDoor>().doorClosing; // Check if door is curently closing.
    }
    ...
    private void OnTriggerEnter(Collider other) // Open door to room. 
    {
        if (enterNum < 1) // Avoid command twice do to sesitivity.
        {
            ...
            closingDoorScript.GetComponent<CloseDoor>().exitNum = 0; // Enable door to close.
            closingDoorScript.GetComponent<CloseDoor>().doorClosing = false; // Now door is opening.
